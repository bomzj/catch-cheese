...
export default class Agent {
    ...
    getState() {
        ...
        var cheeseDirection = playerIndex > cheeseIndex ? -1 : 1;
    ...
    async act() {
        ...
        // Take the best action or explore the map
        if (this.exploration < Math.random()) {
            var state = this.getState();
            var action = this.getBestAction(state);
        } else { 
            action = Math.random() < 0.5 ? 0 : 1;
        }

        // Sleep a bit to emulate slow human response time
        if (!this.isTraining)
    ...
    async handleTraining() {
        ...
        this.oldState = state;
        this.oldAction = this.getBestAction(state);
